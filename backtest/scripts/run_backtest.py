...
====================================
Filters OpenInsider purchase data to officer/director/10% trades filed within 5 days,
...
For 3d/5d/10d, return columns store per-day compounded equivalent returns.
...
INPUT_CSV = "backtest/data/insider_purchases.csv"
OUTPUT_CSV = "backtest/data/backtest_results.csv"
...
def compute_horizon_return_pct(buy_price: float, sell_price: Optional[float], horizon_days: int) -> Optional[float]:
    """
    Return percentage used for targets/analysis.
...
def load_and_filter(csv_path: str) -> pd.DataFrame:
    ...
    logger.info(f"Officer/Director/10%% filter: {len(df):,} / {total:,} kept")
...
def print_summary(df: pd.DataFrame):
    ...
    print("  Note: 3d/5d/10d returns are per-day compounded equivalents (% per trading day).")
