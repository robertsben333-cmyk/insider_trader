...
def fetch_one(api_key: str, cache: PriceCache, ticker: str, d_str: str) -> tuple[str, str, int, str]:
    ...
    target_date = isoparse(d_str).date()
...
def build_missing_pairs(cache: PriceCache) -> pd.DataFrame:
    scored = load_scored_day1()[["ticker", "buy_datetime"]].copy()
    scored["buy_datetime"] = pd.to_datetime(scored["buy_datetime"], errors="coerce")
    scored = scored.dropna(subset=["buy_datetime"]).copy()
    n_train, _split_80 = chrono_split_60_20_20(len(scored))
    subset = scored.iloc[:n_train].copy()
    subset["trade_day"] = subset["buy_datetime"].dt.strftime("%Y-%m-%d")
    pairs = subset[["ticker", "trade_day"]].drop_duplicates().sort_values(["ticker", "trade_day"]).reset_index(drop=True)
    pairs["cache_exists"] = [
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Backfill missing minute cache files for the original all-company training split.")
    parser.add_argument("--cache-dir", default=str(CACHE_DIR))
    parser.add_argument("--workers", type=int, default=8)
    parser.add_argument("--limit", type=int, default=0, help="Optional max number of missing pairs to fetch.")
    parser.add_argument("--manifest-out", default=str(OUT_DIR / "train_minute_cache_backfill_manifest.csv"))
    ...
    pairs = build_missing_pairs(cache)
